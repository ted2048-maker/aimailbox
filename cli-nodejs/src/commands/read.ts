...
export const readCommand = new Command('read')
  ...
  .argument('[index]', 'Message index (1-indexed)', '1')
    ...
    async (
      ...
      try {
        ...
        if (options.latest) {
          ...
          const msgIndex = parseInt(index, 10) - 1;
          if (msgIndex < 0 || msgIndex >= listResult.messages.length) {
            console.error(
              chalk.red(`Invalid message index. Valid range: 1-${listResult.messages.length}`)
            );
            process.exit(1);
          }
          targetMsg = listResult.messages[msgIndex];
        }
